...
export default function BerichtenPage() {
  ...
  const onbeantwoord = gesprekken.filter(g => g.unread > 0);
  const zichtbaar = filter === "onbeantwoord" ? onbeantwoord
    : filter === "archief" ? gesprekken.filter(g => g.gelezen)
    : gesprekken;
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100%", background: "#F5EFE5" }}>
      <div className="px-5 pt-14 pb-4" style={{ background: "rgba(245,239,229,0.97)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16 }}>
          ...
          <button style={{ padding: 8, background: "transparent", border: "0.5px solid #E5DDD0", borderRadius: 99, color: "#1A1D1A", cursor: "pointer", display: "flex", alignItems: "center" }}>
            <PenSquare size={15} />
          </button>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#FBF7F0", border: "0.5px solid #E5DDD0", borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}>
          ...
          <span style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 14, color: "#8A8A83", flex: 1 }}>
            {isVakman ? "Zoek in gesprekken…" : "Zoek vakman of klus…"}
          </span>
        </div>
